...
import { db, handleFirestoreError, OperationType, logActivity, uploadFileToFirestore } from '../firebase';
...
export function InventoryContent({ employees }: InventoryContentProps) {
  ...
  const handleAssignSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (assignProofFile) {
        uploadedUrl = await uploadFileToFirestore(assignProofFile);
      }
                ...
                {!(selectedAsset.history && selectedAsset.history.length > 0) ? (
                  ...
                  selectedAsset.history.slice().reverse().map((hist, idx) => (
                    <div key={hist.id} className="relative flex items-start gap-4">
                      ...
                      <div 
                        className={`flex-1 rounded-xl p-4 border transition-all ${idx === 0 && !hist.dateReturned ? 'border-blue-200 bg-blue-50/50' : 'border-slate-100 bg-white shadow-sm hover:shadow-md'} ${(hist.giverName || hist.proofImageUrl) ? 'cursor-pointer' : ''}`}
                        onClick={() => {
          ...
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden flex flex-col shadow-2xl">
            ...
            <form onSubmit={handleAssignSubmit} className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">Diberikan Oleh (Opsional)</label>
                  <input
                    type="text"
                    value={assignGiver}
                    onChange={(e) => setAssignGiver(e.target.value)}
                    placeholder="Nama yang memberikan barang"
                    className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all text-sm"
                  />
                </div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.files && e.target.files[0]) {
                          setAssignProofFile(e.target.files[0]);
                        }
                      }}
                      className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-600 hover:file:bg-blue-100"
                    />
                  ) : (
              ...
              <div className="mt-8 flex justify-end gap-3">
                ...
                <button 
                  type="submit"
                  disabled={!assignEmployeeId || isUploadingProof}
                  className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl text-sm font-bold shadow-sm disabled:opacity-50 transition-all flex items-center justify-center min-w-[140px]"
                >
                  {isUploadingProof ? (
                    <div className="flex items-center gap-2">
